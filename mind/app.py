...
def generate_images(openpose, pipe):
    ...
    while success:
        ...
        image_output = pipe(f"{focus_cmd} + ' ' + {emotion_cmd}, beautiful, highly insanely detailed, top quality, best quality, 4k, 8k, art single girl character, art like, very high quality", pose_img, negative_prompt="normal quality, low quality, worst quality, jpeg artifacts, username, watermark, signature, time signature, timestamp, artist name, copyright name, copyright, loli, child, infant, baby, bad anatomy, extra hands, extra legs, extra digits, extra_fingers, wrong finger, inaccurate limb, tits, nipple, pubic hair", num_inference_steps=15).images[0]
...
def diffusion_feed_model():
    # Initialize the OpenPose detector and the ControlNet diffusion pipeline
...
def generate_frames(faceCascade):
    while True:
        ...
        if success:
            ...
            # Encode the frame as JPEG bytes for the multipart stream
