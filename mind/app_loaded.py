...
def generate_images(openpose, pipe):
    ...
    while success:
        ...
        image_output = pipe(f"{focus_cmd} + ' ' + {emotion_cmd}, beautiful, highly insanely detailed, top quality, best quality, 4k, 8k, art single girl character, art like, very high quality",
                            pose_img,
                            negative_prompt="normal quality, low quality, worst quality, jpeg artifacts, username, watermark, signature, time signature,\
                                            timestamp, artist name, copyright name, copyright, loli, child, infant, baby, bad anatomy, extra hands, extra legs, extra digits, \
                                            extra_fingers, wrong finger, inaccurate limb, tits, nipple, pubic hair",
...
def start_camera():
    """Run webcam capture on its own thread."""
    global cap
    cap = cv2.VideoCapture(0)
    if not cap.isOpened():
...
# --------------------- Affective image generation (StreamDiffusion) --------
...
def generate_streamdiffusion_images(stream):
    ...
    while True:
        ...
        # Build the generation prompt from the detected attention and emotion state
